...
test.describe('Production Health - Synthetic Monitoring', () => {
    ...
    test('Critical navigation paths work', async ({ page }) => {
        ...
        // Strict assertion: Privacy link must be visible (required for compliance)
    ...
    test('Page loads without JavaScript errors', async ({ page }) => {
        ...
        // Attach listeners BEFORE navigation to capture all errors from initial load
        page.on('console', msg => {
            if (msg.type() === 'error') {
        ...
        // Assert no critical errors occurred (filter known non-critical errors)
        const criticalErrors = errors.filter(err =>
            !err.includes('favicon') && // Favicon not found is non-critical
            !err.includes('AdBlock') && // Ad blocker warnings are non-critical
            !err.includes('Failed to load resource') && // Handled separately by requestfailed listener
            !err.includes('405') // 405 errors are tracked in asset test
        );
