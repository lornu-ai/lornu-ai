...
# Data source for the existing OIDC provider (already created manually)
data "aws_iam_openid_connect_provider" "github" {
  url = "https://token.actions.githubusercontent.com"
}

# GitHub Actions role for ECR + Terraform operations
...
# ECR permissions for GitHub Actions to push images
...
# Terraform Cloud permissions (read/write state, create resources in AWS)
# Note: Terraform API token is used by the workflow, not AWS IAM; this allows
# GitHub Actions to create/modify AWS resources via TF.
resource "aws_iam_policy" "github_terraform_aws" {
  name        = "github-actions-terraform-aws"
  description = "Allow GitHub Actions to apply Terraform configurations"
